...
public class MailInfoData {
	...
	public static MailInfoData convertFromMail(Mail mail)
	{
		...
		mailInfoData.setReceiveId(mail.getRecId());
		mailInfoData.setReceiveName(mail.getRecName());
		if(mail.getRandRewardDataList() != null && mail.getRandRewardDataList().size()>0){
			mailInfoData.setGold(mail.getRandRewardDataList().get(0).getRewardCount());
		}else{
			mailInfoData.setGold(0);
		}
