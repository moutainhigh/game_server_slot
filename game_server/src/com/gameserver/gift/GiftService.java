...
import com.gameserver.gift.template.GiftNewTemplate;
...
public class GiftService implements InitializeRequired{
	...
	private List<GiftNewTemplate> giftNewTemplateList = new ArrayList<GiftNewTemplate>();
	...
	public void init() {
		 ...
		 * 新的用户发送的礼物
		 */
		Map<Integer,GiftNewTemplate> newTemplateMap = Globals.getTemplateService().getAll(GiftNewTemplate.class);
		
		for(Entry<Integer,GiftNewTemplate> en : newTemplateMap.entrySet()){
			   int id = en.getKey();
			   GiftNewTemplate ft = en.getValue();
			   giftNewTemplateList.add(ft);
		}
	...
	public List<GiftNewTemplate> getGiftNewTemplateList() {
		return giftNewTemplateList;
	}

	public void setGiftNewTemplateList(List<GiftNewTemplate> giftNewTemplateList) {
		this.giftNewTemplateList = giftNewTemplateList;
	}
