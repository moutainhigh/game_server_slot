...
public class HumanGCMessage:AbstractMessageReceiver
{
	protected override void collectObservers()
	{
		...
		register(NetMessageType.GC_BANK_STATE,GC_BANK_STATE);
		register(NetMessageType.GC_BANK_SET_PASSWORD,GC_BANK_SET_PASSWORD);
		register(NetMessageType.GC_BANK_VIEW,GC_BANK_VIEW);
		register(NetMessageType.GC_BANK_CHANGE_PASSWORD,GC_BANK_CHANGE_PASSWORD);
		register(NetMessageType.GC_BANK_SEND_IDENTIFYING_CODE,GC_BANK_SEND_IDENTIFYING_CODE);
		register(NetMessageType.GC_BANK_MAKE_SURE_IDENTIFYING_CODE,GC_BANK_MAKE_SURE_IDENTIFYING_CODE);
	}
	...
	public void GC_BANK_CHANGE_PASSWORD(InputMessage data) 
	{
  	...
  	/**
	 * 银行 发送验证码  返回
	 * @param codeState 验证码 (是:0,否:1) 发送成功（每个用户发送验证码的 次数有限，超过次数 就不会 发了）
	 */
	public void GC_BANK_SEND_IDENTIFYING_CODE(InputMessage data) 
	{
		int codeState = data.GetInt();		
		HumanHandler.Instance().GC_BANK_SEND_IDENTIFYING_CODE(codeState);
	}
