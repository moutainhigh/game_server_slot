...
public class MailCGMessage{
	...
	public static void CG_LOAD_MAIL_LIST(int mailKind) {
		 ...
		 * @param mailKind 邮件类型
		 */
	public static void CG_LOAD_SEND_LIST(int mailKind) {
			OutputMessage msgBody= new OutputMessage(NetMessageType.CG_LOAD_SEND_LIST);
			msgBody.PutInt(mailKind);
			PlatformService.Send(msgBody);
		}
	...
	msgBody.PutInt((randReward[i] as RandRewardData).vippoint);
	msgBody.PutInt((randReward[i] as RandRewardData).giftNewId);
			}
			PlatformService.Send(msgBody);
		}
	...
	msgBody.PutLong((long)mailIdList[i]);
		 ...
		 * @param mailIdList 邮件Id列表
		 */
	public static void CG_DELETE_SEND_CANCEL_MAIL(ArrayList mailIdList) {
			OutputMessage msgBody= new OutputMessage(NetMessageType.CG_DELETE_SEND_CANCEL_MAIL);
			int i;
			msgBody.PutShort((short)mailIdList.Count);
			for(i=0; i<mailIdList.Count; i++){
	msgBody.PutLong((long)mailIdList[i]);
