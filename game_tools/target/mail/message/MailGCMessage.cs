...
public class MailGCMessage:AbstractMessageReceiver
{
	...
	{
		register(NetMessageType.GC_LOAD_SEND_LIST,GC_LOAD_SEND_LIST);
		register(NetMessageType.GC_SEND_FINISH,GC_SEND_FINISH);
	 ...
	 */
	public void GC_LOAD_SEND_LIST(InputMessage data) 
	{
		int i,size;
		...
		for(i=0; i<size; i++)
		{
			...
			mailInfoDataList_Datas.receiveId =data.GetLong();
			mailInfoDataList_Datas.receiveName = data.GetString();//收件角色名称
			mailInfoDataList_Datas.gold =data.GetLong();
		...
		MailHandler.Instance().GC_LOAD_SEND_LIST(mailKind,mailInfoDataList);
	}
